...
const banquetFormSchema = z.object({
  name: z.string().min(1, "Banquet name is required"),
  date: z.date({ required_error: "Banquet date is required" }),
  guestCountStr: z.string().min(1, "Guest count is required").refine(val => /^\d+$/.test(val) && parseInt(val) > 0, {
    message: "Guest count must be a positive whole number",
  }),
  menu: z.array(z.object({ recipeId: z.string() })).min(1, "At least one recipe must be selected for the menu"),
});
...
export function BanquetForm({ initialData, allRecipes, onSubmit, isEditing = false }: BanquetFormProps) {
  ...
  return (
    <Card className="max-w-2xl mx-auto shadow-lg">
      <CardHeader>
        <CardTitle>{isEditing ? "Edit Banquet" : "Add New Banquet"}</CardTitle>
        <CardDescription>{isEditing ? "Update the details of your banquet." : "Plan your new banquet event."}</CardDescription>
      </CardHeader>
      <form onSubmit={handleSubmit(processSubmit)}>
        <CardContent className="space-y-6">
          <div>
            <Label htmlFor="name">Banquet Name</Label>
            <Input id="name" {...register("name")} placeholder="e.g., Annual Charity Ball" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="guestCountStr">Number of Guests</Label>
              <div className="relative">
                <Users className="absolute left-2 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                <Input id="guestCountStr" type="number" {...register("guestCountStr")} placeholder="e.g., 50" className="pl-8" min="1" />
              </div>
          ...
          <div>
            <Label className="flex items-center gap-1"><Utensils className="w-4 h-4" /> Menu Selection</Label>
            {allRecipes.length === 0 ? (
              <p className="text-sm text-muted-foreground mt-2">No recipes available. Please <Link href="/recipes/new" className="text-primary underline">add a recipe</Link> first.</p>
            ) : (
        ...
        <CardFooter className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={allRecipes.length === 0 && !isEditing}>{isEditing ? "Save Changes" : "Add Banquet"}</Button>
        </CardFooter>
      </form>
    </Card>
  );
